...
select s.sname from sailors as s where s.sid in (select r.sid from reserves as r where r.bid = (select b.bid from boats as b where b.bid = r.bid and b.color = 'red')); 
select s.sname from sailors as s join reserves as r  on s.sid = r.sid join boats as b on r.bid = b.bid where b.color = 'red'; 
